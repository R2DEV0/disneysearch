        ...
        <div className='row'>
          {loaded && 
            <div className='resultsContainer col-sm-12' style={{display: 'flex', flexWrap: 'wrap', justifyContent: 'center'}}>
              {searchResults.map((item, i) => { return <div key={i} className="col-lg-4 col-md-6 col-sm-12" style={{height: '500px', border: '1px solid gray', borderRadius: '5px', boxShadow: '3px 3px 5px gray'}}>
                <ul style={{listStyle:'none', textAlign: 'center'}}>
                  <li style={{fontWeight: '800'}}><h2>{item.name}</h2></li>
                  <li> <img src={item.imageUrl} width={100} height={auto}/> </li>
                </ul>
              </div>})}
            </div>
          }
